...
export interface User {
  id: EntityId
  avatar: unknown // show "any"
  age: unknown
  subsidiary: Subsidiary
  alternativeAccounts: User[]
}
...
// 2. Implementing the fetch function

export async function fetchUserSubsidiaryName (): Promise<string> {
  const user = await fetchB7User<User>()
  return user.subsidiary.name
}

// 3. Adding the non-implemented function

export async function fetchUserAge (): Promise<never> {
